...
for ii = 1:length(allnames)
    ...
    all_rr1nSEM_microns=[all_rr1nSEM_microns rr1a_microns_vesselSEM_Final];
    
    all_rr1a_percent=[all_rr1a_percent rr1a_percent_vessel_Final]; 
    all_rr1nSEM_percent=[all_rr1nSEM_percent rr1a_percent_vesselSEM_Final];
    
    all_rr2a_microns = [all_rr2a_microns rr2a_microns_vessel_Final];
    all_rr2nSEM_microns=[all_rr2nSEM_microns rr2a_microns_vesselSEM_Final];
    
    all_rr2a_percent=[all_rr2a_percent rr2a_percent_vessel_Final]; 
    all_rr2nSEM_percent=[all_rr2nSEM_percent rr2a_percent_vesselSEM_Final];
...
save(groupname,'allnames','all_rr1a_microns','all_rr1nSEM_microns','all_rr1a_percent',...
    'all_rr1nSEM_percent','all_rr2a_microns','all_rr2nSEM_microns','all_rr2a_percent',...
    'all_rr2nSEM_percent','all_yproj2max_percent','all_yproj2max_SEMpercent','tt')
    


%%
mean_rr1a_microns=mean(all_rr1a_microns,2);
mean_rr1aSEM_microns=mean(all_rr1nSEM_microns,2);

mean_rr1a_percent=mean(all_rr1a_percent,2);
mean_rr1aSEM_percent=mean(all_rr1nSEM_percent,2);

mean_rr2a_microns=mean(all_rr2a_microns,2);
mean_rr2aSEM_microns=mean( all_rr2nSEM_microns,2)

mean_rr2a_percent=mean(all_rr2a_percent,2);
mean_rr2aSEM_percent=mean(all_rr2nSEM_percent,2)
